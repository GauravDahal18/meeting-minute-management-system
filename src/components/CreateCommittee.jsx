...
const CreateCommitteeDialog = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/api/getAllMembers`, {
                          ...
                          {(() => {
                            const membersToShow =
                              showSearchResults && searchTerm
                                ? filteredMembers
                                : membersData;
                            const addedMembers = membersToShow.filter(
                              (member) =>
                                committeeMembership.find(
                                  (m) => m.memberId === member.memberId
                                )
                            );
                            const nonAddedMembers = membersToShow.filter(
                              (member) =>
                                !committeeMembership.find(
                                  (m) => m.memberId === member.memberId
                                ) &&
                            ...
                            return sortedMembers.map((member) => {
                              ...
                              const isAlreadyAdded = committeeMembership.find(
                                (m) => m.memberId === member.memberId
                              );
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            let roleValue = roleDropdown.value;
                                            ...
                                            // Check if the role is a special role and already assigned
                                            const isSpecialRole =
                                              specialRoles.includes(roleValue);
                                            if (isSpecialRole) {
              ...
              <div className="col-span-1 space-y-5">
                ...
                >
                  ...
                  <div className="mb-4">
                    ...
                    >
                      Description/Aim <span className="text-red-500">*</span>
                    </label>
                    <textarea
