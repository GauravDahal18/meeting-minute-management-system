...
const CreateMeetingDialog = () => {
  ...
  const removeInvitee = (id) => {
    setAddedInviteeIds((prev) => prev.filter((inviteeId) => inviteeId !== id));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(
      "API endpoint:",
      `${BASE_URL}/api/createMeeting?committeeId=${committeeId}`
    );
                ...
                >
                  ...
                  >
                    <ul className="space-y-3">
                      {filteredInvitees
                        .filter(
                          (invitee) => !addedInviteeIds.includes(invitee.id)
                        )
                        .map((invitee) => (
                          <li
                            key={invitee.id}
                          ...
                          >
                            <span
                              onClick={() => navigate(`/member/${invitee.id}`)}
